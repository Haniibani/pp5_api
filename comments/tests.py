...
class CommentTests(TestCase):
    ...
    def test_comment_list_retrieval(self):
        ...
        Comment.objects.create(owner=self.user1, post=self.post, content='Nice post!')
        self.client.login(username='user1', password='password123')
    ...
    def test_comment_update(self):
        ...
        comment = Comment.objects.create(owner=self.user1, post=self.post, content='Nice post!')
        self.client.login(username='user1', password='password123')
        response = self.client.patch(f'/comments/{comment.id}/', {'content': 'Updated content'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
